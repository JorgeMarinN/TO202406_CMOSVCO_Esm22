...
df=pd.read_fwf('data_TDBuckTOP-CL_v5p2_RL6p67.txt',infer_nrows = 1000)
df.to_csv('data_TDBuckTOP-CL_v5p2_RL6p67.csv', index=False)
data = pd.read_csv("data_TDBuckTOP-CL_v5p2_RL6p67.csv").values
...
ih_v = y_ih[peaks[pk_init]:peaks[pk_len-1]]
xih_v = x_ih[peaks[pk_init]:peaks[pk_len-1]]
plt.plot(xih_v,ih_v)
plt.show()
